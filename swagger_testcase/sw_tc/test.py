...
class AnalysisSwaggerJson(object):
    ...
    def analysis_json_data(self, isDuplicated=False):
        ...
        if "9527" in self.url:
            try:
                res = requests.get(self.url + '/v2/api-docs?group=全量接口').json()  # 这才是swagger接口请求的地址
            ...
            try:
                res = requests.get(self.url + '/v2/api-docs').json()  # 这才是swagger接口请求的地址
                write_data(res, 'data.json')
            except Exception as e:
                log.error('请求swagger地址错误. 异常如下: {}'.format(e))
                raise e
        ...
        self.url = 'http://' + res['host']  # 第一错，swagger文档是ip地址，使用https协议会错误,注意接口地址的请求协议
        ...
        if isinstance(self.data, dict):  # 判断接口返回的paths数据类型是否dict类型
            for tag in self.tags_list:  # 前面已经把接口返回的结果tags分别写入了tags_list空列表,再从json对应的tag往里面插入数据
                self.http_case = {"config": {"name": "", "base_url": "", "variables": {}}, "teststeps": []}
                for key, value in self.data.items():
                    for method in list(value.keys()):
                        params = value[method]
                        if not 'deprecated' in value.keys():  # deprecated字段标识：接口是否被弃用，暂时无法判断，使用consumes偷换
    ...
    def wash_params(self, params, api, method, tag):
        ...
        name = params['summary'].replace('/', '_')  # 这里的问题需要具体来分析,开发有时概要使用其他符号分割
        http_interface['name'] = name
        http_api_testcase['name'] = name
        http_api_testcase['api'] = 'api/{}/{}.json'.format(tag, name)  # 这是写入testcasejson下的名字,不是生成api的目录
    ...
    def write_excel(self, url, filelist):
        ...
        for file in filelist:
            caseId += 1
            count += 1
            inter_name = file.split("\\")[-2]  # 获取接口测试用例的上级目录名称：组成name-tag的用例title
            with open(file, 'r', encoding='utf-8') as rdfile:
                ...
                if 'json' in text['request'].keys():  # post请求的接口相关数据写入excel
                    ...
                    join_text = jsonp.replace("{", "").replace("}", "").replace(":", "=").replace("'", "").replace(",",
                                                                                                                   "&").replace(
                        " ", "")
                    w.write(count, 7, uri + url)
